...
function errorId(field: EnquiryFieldName) {
...
export function EnquiryForm({ onSuccess }: EnquiryFormProps) {
  ...
  const inputClassName = "mt-1.5 min-h-11 w-full rounded-lg border border-border bg-surface/90 px-3 text-sm text-text-primary outline-none transition-colors placeholder:text-text-secondary/70 focus:border-primary focus:ring-2 focus:ring-primary/25 disabled:cursor-not-allowed disabled:opacity-60";

  return (
    <form className="mt-6 space-y-4" noValidate onSubmit={handleSubmit}>
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="enquiry-full-name" className="text-sm font-semibold text-text-primary">{ENQUIRY_CONTENT.fields.fullName.label}</label>
          <input id="enquiry-full-name" name="fullName" autoComplete="name" maxLength={ENQUIRY_LIMITS.fullName} disabled={isSending || isSuccess} value={values.fullName} onChange={(event) => updateValue("fullName", event.target.value)} placeholder={ENQUIRY_CONTENT.fields.fullName.placeholder} aria-invalid={Boolean(errors.fullName)} aria-describedby={errors.fullName ? errorId("fullName") : undefined} className={inputClassName} />
          {errors.fullName ? <p id={errorId("fullName")} className="mt-1 text-xs text-danger">{errors.fullName}</p> : null}
        </div>
        <div>
          <label htmlFor="enquiry-email" className="text-sm font-semibold text-text-primary">{ENQUIRY_CONTENT.fields.email.label}</label>
          <input id="enquiry-email" name="email" type="email" autoComplete="email" maxLength={ENQUIRY_LIMITS.email} disabled={isSending || isSuccess} value={values.email} onChange={(event) => updateValue("email", event.target.value)} placeholder={ENQUIRY_CONTENT.fields.email.placeholder} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? errorId("email") : undefined} className={inputClassName} />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="enquiry-phone" className="text-sm font-semibold text-text-primary">{ENQUIRY_CONTENT.fields.phone.label}</label>
          <div className="relative mt-1.5 flex min-h-11 rounded-lg border border-border bg-surface/90 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/25">
            <div className="relative shrink-0 border-r border-border">
              <button type="button" onClick={() => setIsCountryMenuOpen((open) => !open)} disabled={isSending || isSuccess} aria-expanded={isCountryMenuOpen} aria-haspopup="listbox" className="flex min-h-11 items-center gap-1 px-3 text-sm font-semibold text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              ...
              {isCountryMenuOpen ? (
                <div className="absolute top-[calc(100%+0.5rem)] left-0 z-20 w-72 rounded-lg border border-border bg-surface p-2 shadow-xl">
                  <label className="sr-only" htmlFor="enquiry-country-search">{ENQUIRY_CONTENT.countrySearchLabel}</label>
                  <input ref={searchInputRef} id="enquiry-country-search" type="search" value={countrySearch} onChange={(event) => setCountrySearch(event.target.value)} placeholder={ENQUIRY_CONTENT.countrySearchPlaceholder} className="min-h-11 w-full rounded-md border border-border px-3 text-sm text-text-primary outline-none focus:border-primary focus:ring-2 focus:ring-primary/25" />
        ...
        <div>
          <label htmlFor="enquiry-program" className="text-sm font-semibold text-text-primary">{ENQUIRY_CONTENT.fields.program.label}</label>
          <div className="relative">
            <select id="enquiry-program" name="program" disabled={isSending || isSuccess} value={values.program} onChange={(event) => updateValue("program", event.target.value)} aria-invalid={Boolean(errors.program)} aria-describedby={errors.program ? errorId("program") : undefined} className={`${inputClassName} appearance-none pr-11`}>
      ...
      <div>
        <label htmlFor="enquiry-message" className="text-sm font-semibold text-text-primary">{ENQUIRY_CONTENT.fields.message.label}</label>
        <textarea id="enquiry-message" name="message" rows={3} maxLength={ENQUIRY_LIMITS.message} disabled={isSending || isSuccess} value={values.message} onChange={(event) => updateValue("message", event.target.value)} placeholder={ENQUIRY_CONTENT.fields.message.placeholder} aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? errorId("message") : undefined} className={`${inputClassName} min-h-24 resize-y py-3`} />
